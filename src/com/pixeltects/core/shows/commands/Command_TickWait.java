...
public class Command_TickWait implements CommandExecutor {

    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (cmd.getName().equalsIgnoreCase("tickwait")) {
            if(sender.isOp() || sender.hasPermission("pixeltects.show.tickwait")
            || sender.hasPermission("pixeltects.show")) {
                if(args.length == 0) {
                    sender.sendMessage(ChatColor.RED + "Correct usage: /tickwait <ticksToWait> <command>");
